use crate::config::{Config, Operation, FilterOperation, Stroke, Shadow};
use anyhow::Result;
use image::{DynamicImage, Rgba, ImageFormat, imageops::colorops};
...
impl ImageProcessor {
    ...
    pub fn save_image(img: &DynamicImage, output_config: &crate::config::OutputConfig) -> Result<()> {
    ...
    fn determine_format(output_config: &crate::config::OutputConfig) -> ImageFormat {
        if let Some(ref format) = output_config.format {
            match format.to_lowercase().as_str() {
                ...
                _ => ImageFormat::from_path(&output_config.destination)
                    .unwrap_or(ImageFormat::Png),
            }
        } else {
            ImageFormat::from_path(&output_config.destination)
                .unwrap_or(ImageFormat::Png)
        }
    }
    ...
    fn apply_operation(img: &DynamicImage, operation: &Operation) -> Result<DynamicImage> {
        match operation {
            ...
            Operation::Overlay { image: overlay_path, x, y, opacity, blend_mode: _ } => {
            ...
            Operation::Filter(filter_op) => {
                Self::apply_filter_operation(img, filter_op)
            }
            
            Operation::Text { content, font, size, color, x, y, stroke, shadow } => {
                Self::draw_text(img, content, font, *size, color, *x, *y, stroke.as_ref(), shadow.as_ref())
            }
        }
    }
    
    fn apply_filter_operation(img: &DynamicImage, filter_op: &FilterOperation) -> Result<DynamicImage> {
        match filter_op {
            FilterOperation::Grain { intensity } => Ok(Self::add_grain(img, *intensity)),
            FilterOperation::Blur { radius } => Ok(img.blur(*radius)),
            FilterOperation::DoubleVision { offset_x, offset_y, opacity } => {
    ...
    fn overlay_with_opacity(base: &mut DynamicImage, overlay: &DynamicImage, x: i32, y: i32, opacity: f32) {
        ...
        for ox in 0..overlay_width {
            for oy in 0..overlay_height {
                ...
                if base_x >= 0 && base_x < result.width() as i32 && base_y >= 0 && base_y < result.height() as i32 {
                    ...
                    if overlay_pixel[3] > 0 {
                        ...
                        let blended = [
                            (base_pixel[0] as f32 * (1.0 - alpha) + overlay_pixel[0] as f32 * alpha) as u8,
                            (base_pixel[1] as f32 * (1.0 - alpha) + overlay_pixel[1] as f32 * alpha) as u8,
                            (base_pixel[2] as f32 * (1.0 - alpha) + overlay_pixel[2] as f32 * alpha) as u8,
                            (base_pixel[3] as f32 * (1.0 - alpha) + overlay_pixel[3] as f32 * alpha) as u8,
                        ];
    ...
    fn double_vision(img: &DynamicImage, offset_x: i32, offset_y: i32, opacity: f32) -> DynamicImage {
        ...
        for x in 0..width {
            for y in 0..height {
                ...
                if src_x >= 0 && src_x < width as i32 && src_y >= 0 && src_y < height as i32 {
                    ...
                    current_pixel[0] = (current_pixel[0] as f32 * (1.0 - opacity) + ghost_pixel[0] as f32 * opacity) as u8;
                    current_pixel[1] = (current_pixel[1] as f32 * (1.0 - opacity) + ghost_pixel[1] as f32 * opacity) as u8;
                    current_pixel[2] = (current_pixel[2] as f32 * (1.0 - opacity) + ghost_pixel[2] as f32 * opacity) as u8;
